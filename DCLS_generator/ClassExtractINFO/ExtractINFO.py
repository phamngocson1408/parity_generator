...
class ExtractINFO:
    ...
    def _read_info(self):
        info_df = pd.read_excel(self.INFO_path)
        info_df.dropna(how='all', axis=0, inplace=True)  # Remove empty rows
        info_df.dropna(how='all', axis=1, inplace=True)  # Remove empty columns
    ...
    def _read_info_multi(self):
        if self.sheet_name:
            info_df = pd.read_excel(self.INFO_path, sheet_name=self.sheet_name)
        else:
            info_df = pd.read_excel(self.INFO_path)
        info_df.dropna(how='all', axis=0, inplace=True)  # Remove empty rows
        info_df.dropna(how='all', axis=1, inplace=True)  # Remove empty columns
